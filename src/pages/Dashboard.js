    ...
    <div>
      ...
      <div style={{
        ...
        <div>
          ...
          {leaderName ? (
            <div style={{ fontSize: 36, fontWeight: 800, color: leaderName === 'Bill' ? 'var(--blue-light)' : 'var(--red)', lineHeight: 1 }}>
              {leaderName} leads <span style={{ color: 'var(--green)' }}>${leaderAmt}</span>
            </div>
          ) : (
            <div style={{ fontSize: 36, fontWeight: 800, color: 'var(--text-muted)' }}>Even</div>
          )}
          ...
          <>
              ...
              <table className="data-table">
                <thead>
                  <tr>
                    ...
                    <th className="num" style={{ width: 130 }}>Result</th>
                    <th className="num" style={{ width: 150 }}>Running Total</th>
                  ...
                  {reversedWeeks.map((w) => {
                    ...
                    return (
                      <tr key={w.id} onClick={() => onEditWeek(w.id)} style={{ cursor: 'pointer' }}>
                        ...
                        <td>
                          {w.completed ? <span className="badge badge-green">Done</span> : <span className="badge badge-yellow">In Progress</span>}
                        </td>
                        <td className="num">
                          {amt != null ? (
            ...
            <div className="mobile-only" style={{ padding: '8px 0' }}>
              {reversedWeeks.map((w) => {
                ...
                const rt      = w.runningTotal || 0;
                return (
                  <div key={w.id} onClick={() => onEditWeek(w.id)} style={{
                    padding: '12px 16px', borderBottom: '1px solid var(--navy-border)',
                    cursor: 'pointer', display: 'flex', justifyContent: 'space-between', alignItems: 'center'
                  }}>
                    <div>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 4 }}>
                        <span style={{ fontSize: 12, color: 'var(--text-muted)', fontWeight: 600 }}>#{w.raceNum}</span>
                        {w.firstPick && <span style={{ fontWeight: 800, fontSize: 13, color: w.firstPick === 'Bill' ? 'var(--blue-light)' : 'var(--red)' }}>{w.firstPick === 'Bill' ? 'B' : 'D'}</span>}
                        <span style={{ fontWeight: 700, color: 'var(--blue-pale)', fontSize: 15 }}>{w.track || 'Unnamed'}</span>
                        {w.completed ? <span className="badge badge-green">Done</span> : <span className="badge badge-yellow">In Progress</span>}
                      </div>
                      <div style={{ fontSize: 12, color: 'var(--text-muted)' }}>
                        {w.raceDate ? new Date(w.raceDate + 'T12:00:00').toLocaleDateString('en-US', { month: 'short', day: 'numeric' }) : ''}
                        {amt != null && <span style={{ marginLeft: 8, fontWeight: 700, color: billWon ? 'var(--blue-light)' : donWon ? 'var(--red)' : 'var(--text-muted)' }}>
                          {billWon ? 'Bill' : donWon ? 'Don' : 'Even'}{amt > 0 ? ` +$${amt}` : ''}
                        </span>}
                      </div>
                    </div>
                    <div style={{ textAlign: 'right' }}>
                      {r && <div style={{ fontSize: 14, fontWeight: 800, color: rt > 0 ? 'var(--blue-light)' : rt < 0 ? 'var(--red)' : 'var(--text-muted)' }}>
                        {rt > 0 ? `Bill +$${rt}` : rt < 0 ? `Don +$${Math.abs(rt)}` : 'Even'}
                      </div>}
                    </div>
                  </div>
