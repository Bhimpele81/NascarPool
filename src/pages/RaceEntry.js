...
function MoneyBreakdown({ label, result, side }) {
  ...
  const myStreak    = side === 'bill' ? result.billStreakLen    : result.donStreakLen;
    ...
    <div>
      ...
      <table style={{ width: '100%', fontSize: 13 }}>
        <tbody>
          ...
          <MRow label={`Point diff (${diff >= 0 ? '+' : ''}${diff.toFixed(2)}) ÷ 3`} val={fromPts} />
          {myStreakB > 0 && <MRow label={`🏆 Consecutive winner bonus (${myStreak} weeks)`} val={myStreakB} />}
          {oppStreakB > 0 && <MRow label={`Opponent consecutive winner bonus`} val={-oppStreakB} />}
