        ...
        <table className="data-table">
          ...
          <tbody>
            {drivers.map((d, i) => {
              ...
              return (
                <tr key={d.id} style={isWinner ? { background: 'rgba(245,158,11,0.08)' } : {}}>
                  <td><span className={`tier-label ${tierClass(i)}`}>{tierLabel(i)}</span></td>
                  <td>
                    <select
                  ...
                  <td className="num">{pts.top10Pts > 0 ? <span style={{ color: 'var(--green)', fontWeight: 700 }}>{pts.top10Pts}</span> : <span style={{ color: 'var(--navy-border)' }}>0</span>}</td>
                  <td className="num">{pts.stagePts > 0 ? <span style={{ color: '#c084fc', fontWeight: 700 }}>{pts.stagePts}</span> : <span style={{ color: 'var(--navy-border)' }}>0</span>}</td>
                  <td className="num" style={{ color: 'var(--text-muted)', fontSize: 12 }}>{d.finish ? pts.multipliedPoints.toFixed(1) : '—'}</td>
                  <td className="num">
                    {d.finish ? <strong style={{ fontSize: 14, color: isWinner ? 'var(--yellow)' : 'var(--text)' }}>{pts.total.toFixed(1)}</strong> : <span style={{ color: 'var(--navy-border)' }}>—</span>}
